...
static void swift_task_cancelImpl(AsyncTask *task) {
  ...
  // Technically, more children could be added to the task between when
  // we've just set the cancelled bit and when we grab the status record
  // lock to propagate it but updateNewChildWithParentAndGroupState()
  // handles this for the childre since we synchronize with parent's
  // active task status and will see the canclled bit.
...
static swift_task_escalateImpl(AsyncTask *task, JobPriority newPriority) {
  ...
  // lock but updateNewChildWithParentAndGroupState() handles this for the
  // childre since we synchronize with parent's active task status and will
  // see the canclled bit.
