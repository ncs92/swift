...
class alignas(sizeof(void*) * 2) ActiveTaskStatus {
  ...
  enum : uint32_t {
    ...
    /// actor. This bit is only clear when a task stops running and becomes
    /// suspended, or when it is completed.
    ///
    /// Basically, "enqueued" means that the task is runnable or running. Using
    /// same terminology and semantics as logic in dispatch
    IsEnqueued = 0x1000,
  };
...
#if SWIFT_CONCURRENCY_ENABLE_PRIORITY_ESCALATION
    return ActiveTaskStatus(Record, Flags | IsEnqueued, DrainLock);
#else
...
#if SWIFT_CONCURRENCY_ENABLE_PRIORITY_ESCALATION
  return ActiveTaskStatus(newRecord, Flags & ~IsStatusRecordLocked, DrainLock);
#else
    ...
    while (true) {
      ...
      if (task->_private().Status.compare_exchange_weak(oldStatus, newStatus,
        ...
        if (oldStatus.isStoredPriorityEscalated()) {
          ...
          dispatch_lock_override_end((qos_class_t) oldStatus.getStoredPriority());
        }
#endif
        break;
      }
...
inline void AsyncTask::flagAsRunning() {
  ...
  bool canOverride = (dispatch_thread_get_current_qos_override_floor(&overrideFloor) == 0);
...
#if SWIFT_CONCURRENCY_ENABLE_PRIORITY_ESCALATION
    // Task's priority is greater than the thread's - do a self override.
    qos_class_t maxTaskPriority = (qos_class_t) oldStatus.getStoredPriority();
    if (canOverride && (maxTaskPriority > overrideFloor)) {
      ...
      (void) dispatch_thread_override_self(maxTaskPriority);
      overrideFloor = maxTaskPriority;
      goto retry;
    }
#endif

    // Set self as drainer and remove received override bit if any - the task's
    // override has already been reflected on the thread.
...
// TODO (rokhinip): Change the semantics of this function to convey what
// the next state of the task is going to be. If it is to be enqueued
// somewhere else, then we need to make sure that the override on the
// draining thread still exists until the enqueue of the task to the new
// location completes.
inline void AsyncTask::flagAsSuspended() {
  ...
  while (true) {
    ...
    // Remove drain lock and received override bit if any
    newStatus = oldStatus.withRunning(false);
    if (newStatus.isStoredPriorityEscalated()) {
      newStatus = newStatus.withoutStoredPriorityEscalation();

      // TODO (rokhinip): Move this to when the job gets enqueued
